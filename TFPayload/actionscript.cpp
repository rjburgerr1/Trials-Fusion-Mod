...
namespace ActionScript {
    ...
    // Helper function to get checkpoint array info from game manager
    struct CheckpointArrayInfo {
        void** checkpointArray;
        int count;
        bool valid;
    };

    CheckpointArrayInfo GetCheckpointArray() {
        CheckpointArrayInfo info = { nullptr, 0, false };

        if (!gameModule) return info;

        uintptr_t base = reinterpret_cast<uintptr_t>(gameModule);
        uintptr_t* pGamePtr = reinterpret_cast<uintptr_t*>(base + (0x0174b308 - GAME_BASE));
        if (!pGamePtr || *pGamePtr == 0) return info;

        uintptr_t gameBase = *pGamePtr;
        void** ppGameManager = reinterpret_cast<void**>(gameBase + 0xdc);
        if (!ppGameManager || !*ppGameManager) return info;

        void* gameManager = *ppGameManager;
        uintptr_t* mgr = reinterpret_cast<uintptr_t*>(gameManager);

        // Get the count first
        int count = mgr[0x940 / 4];

        if (count <= 0 || count > 100) {
            return info; // Invalid count
        }

        // The value at 0x930 is a pointer to a structure
        // Let's try reading it as a vector: [begin_ptr, end_ptr, capacity_ptr]
        uintptr_t structPtr = mgr[0x930 / 4];

        if (structPtr > 0x10000 && structPtr < 0x7fffffff) {
            uintptr_t* structData = reinterpret_cast<uintptr_t*>(structPtr);

            // Try vector layout: first element should be begin pointer
            void** beginPtr = reinterpret_cast<void**>(structData[0]);

            if (beginPtr && (uintptr_t)beginPtr > 0x10000) {
                // Validate by checking if first element looks like a checkpoint
                void* firstCheckpoint = beginPtr[0];
                if (firstCheckpoint && (uintptr_t)firstCheckpoint > 0x10000) {
                    // Try reading vtable
                    uintptr_t vtable = *reinterpret_cast<uintptr_t*>(firstCheckpoint);
                    if (vtable > 0x10000 && vtable < 0x7fffffff) {
                        // Looks valid!
                        info.checkpointArray = beginPtr;
                        info.count = count;
                        info.valid = true;
                        return info;
                    }
        ...
        // Alternative: Try offset 0x93C which also showed count=14
        // From debug: Offset 0x93c: ptr=0xe7cfe20, count at +0x8 = 14
        uintptr_t altPtr = mgr[0x93C / 4];
        if (altPtr > 0x10000 && altPtr < 0x7fffffff) {
            void** directArray = reinterpret_cast<void**>(altPtr);

            void* firstCheckpoint = directArray[0];
            if (firstCheckpoint && (uintptr_t)firstCheckpoint > 0x10000) {
                uintptr_t vtable = *reinterpret_cast<uintptr_t*>(firstCheckpoint);
                if (vtable > 0x10000 && vtable < 0x7fffffff) {
                    info.checkpointArray = directArray;
                    info.count = count;
                    info.valid = true;
                    return info;
                }
    ...
    bool SetCurrentCheckpoint(int checkpointIndex) {
        LOG_VERBOSE("[ActionScript] SetCurrentCheckpoint - Setting current checkpoint to " << checkpointIndex);

        if (!gameModule || !GetFirstEntity) {
            LOG_ERROR("[ActionScript] Not initialized");
            return false;
        }
        ...
        CheckpointArrayInfo cpInfo = GetCheckpointArray();

        if (!cpInfo.valid) {
            LOG_ERROR("[ActionScript] Failed to get checkpoint array");
            return false;
        }

        if (checkpointIndex < 0 || checkpointIndex >= cpInfo.count) {
            LOG_ERROR("[ActionScript] Checkpoint " << checkpointIndex << " out of range");
            return false;
        }

        void* checkpointPtr = cpInfo.checkpointArray[checkpointIndex];

        if (!checkpointPtr) {
            LOG_ERROR("[ActionScript] Checkpoint " << checkpointIndex << " is null");
            return false;
        }

        LOG_VERBOSE("[ActionScript] Found checkpoint " << checkpointIndex
            << " at 0x" << std::hex << reinterpret_cast<uintptr_t>(checkpointPtr) << std::dec);

        // Get the game manager
        uintptr_t* pGamePtr = reinterpret_cast<uintptr_t*>(base + (0x0174b308 - GAME_BASE));
        if (!pGamePtr || *pGamePtr == 0) return false;

        uintptr_t gameBase = *pGamePtr;
        void** ppGameManager = reinterpret_cast<void**>(gameBase + 0xdc);
        if (!ppGameManager || !*ppGameManager) return false;

        void* gameManager = *ppGameManager;

        // Get the player rider
        void* playerRider = GetFirstEntity(gameManager);

        if (!playerRider) {
            LOG_ERROR("[ActionScript] Player rider is null (not in a race?)");
            return false;
        }

        LOG_VERBOSE("[ActionScript] Player rider at 0x" << std::hex
            << reinterpret_cast<uintptr_t>(playerRider) << std::dec);

        // Set the checkpoint pointer at offset 0x1dc
        void** pCurrentCheckpoint = reinterpret_cast<void**>((uintptr_t)playerRider + 0x1dc);
        *pCurrentCheckpoint = checkpointPtr;

        LOG_VERBOSE("[ActionScript] Set current checkpoint to " << checkpointIndex
            << " (next respawn will be here)");
        return true;
    }
